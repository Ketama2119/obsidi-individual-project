...
public class Resident extends AccountHolder {
    ...
    public static List<Resident> loadResidents() {
        ...
        try {
            ...
            System.out.println( "Error loading residents from file.");
            e.printStackTrace();
        }
        return residents;
    }
        ...
        try {
            ...
            for (List<String> row : rows) {
                ...
                if (row.size() > 2 && row.get(2).equals(residentEmail)) {
                    ...
                    System.out.println("Resident assignment updated successfully." );
                    break;
                }
            }
            rw.writeAll("./file/residents.csv", rows);
        } catch (IOException e) {
            System.out.println( "Error updating resident assignment in file." );
            e.printStackTrace();
        }
    }
    ...
    // Method to print the header for the table in green
